...
func resourceLtsStructTemplateCreate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	if d.Get("template_type").(string) == "custom" {
		url = "v2/" + cfg.HwClient.ProjectID + "/lts/struct/template"
		...
		url = "v3/" + cfg.HwClient.ProjectID + "/lts/struct/template"
	...
	client.WithMethod(httpclient_go.MethodPost).WithUrlWithoutEndpoint(cfg, "lts", cfg.GetRegion(d), url).WithHeader(header).WithBody(opts)
...
func resourceLtsStructTemplateRead(_ context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	header := make(map[string]string)
	header["content-type"] = "application/json;charset=UTF8"
	client.WithMethod(httpclient_go.MethodGet).WithUrlWithoutEndpoint(cfg, "lts", cfg.GetRegion(d), "v2/"+
		cfg.HwClient.ProjectID+"/lts/struct/template?logGroupId="+
...
func resourceLtsStructTemplateDelete(_ context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	client.WithMethod(httpclient_go.MethodDelete).WithUrlWithoutEndpoint(cfg, "lts", cfg.GetRegion(d), "v2/"+
		cfg.HwClient.ProjectID+"/lts/struct/template").
...
func resourceLtsStructTemplateUpdate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	client.WithMethod(httpclient_go.MethodPut).WithUrlWithoutEndpoint(cfg, "lts", cfg.GetRegion(d), "v3/"+
		cfg.HwClient.ProjectID+"/lts/struct/template").
